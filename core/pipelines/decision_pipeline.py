...
from models import Signal, MarketSnapshot, SignalType, normalize_regime_family
...
class DecisionPipeline:
    ...
    def __init__(self, ctx):
        ...
        # Structure Reset Shadow Observer — prospective telemetry for SR tier-sizing experiment
        # Architecturally isolated: has NO OMS/broker imports, writes JSONL only.
        try:
            from core.shadow_sr_observer import StructureResetShadowObserver
            self.sr_shadow_observer = StructureResetShadowObserver()
            logger.info("[SHADOW-SR] Structure Reset shadow observer initialized")
        except Exception as e:
            logger.error(f"[SHADOW-SR] Failed to initialize SR shadow observer: {e}")
            self.sr_shadow_observer = None
    ...
    def _verify_regime_invariant(self, snapshot: MarketSnapshot, signal: Signal) -> dict:
        """
        Diagnostic Invariant: Verifies that RegimeStateManager and Execution Context
        share compatible canonical regime families during active trade evaluation.
        Runs strictly when execution evaluation actually occurs (non-NO_TRADE signals).
        Distinguishes UNVERIFIED_MISSING from actual DRIFT.
        """
        # 1. Extract manager regime
        manager_regime = None
        if hasattr(snapshot, "regime_state") and isinstance(snapshot.regime_state, dict):
            manager_regime = snapshot.regime_state.get("regime")
        elif hasattr(self.ctx, "data_manager") and hasattr(self.ctx.data_manager, "regime_classifier"):
            confirmed = getattr(self.ctx.data_manager.regime_classifier, "current_confirmed_state", None)
            if confirmed and hasattr(confirmed, "regime"):
                manager_regime = confirmed.regime.value if hasattr(confirmed.regime, "value") else str(confirmed.regime)

        # 2. Extract execution regime
        execution_regime = None
        if hasattr(signal, "regime") and signal.regime:
            execution_regime = signal.regime.value if hasattr(signal.regime, "value") else str(signal.regime)

        norm_manager = normalize_regime_family(manager_regime)
        norm_exec = normalize_regime_family(execution_regime)

        if norm_manager is None or norm_exec is None:
            status = "UNVERIFIED_MISSING"
        elif norm_manager == norm_exec:
            status = "PASS"
        else:
            status = "DRIFT"
            logger.warning(
                f"⚠️ [REGIME INVARIANT DRIFT] Manager regime: {manager_regime} ({norm_manager}) "
                f"!= Execution regime: {execution_regime} ({norm_exec}) for signal {getattr(signal, 'id', 'unknown')}"
            )

        invariant_record = {
            "checked": True,
            "manager_regime": str(manager_regime) if manager_regime is not None else None,
            "execution_regime": str(execution_regime) if execution_regime is not None else None,
            "normalized_manager_regime": norm_manager,
            "normalized_execution_regime": norm_exec,
            "status": status
        }

        if not hasattr(signal, "metadata") or signal.metadata is None:
            signal.metadata = {}
        signal.metadata["regime_invariant"] = invariant_record
    ...
    def _log_canonical_truth(self, signal: Signal, cycle_count: int, auth: bool, reason: str, latencies: dict):
        ...
        # Determine structured decision_status: "AUTHORIZED", "HALTED", or "REJECTED"
        if auth:
            decision_status = "AUTHORIZED"
        else:
            is_halted = False
            # 1. Structured signal metadata
            if hasattr(signal, "metadata") and isinstance(signal.metadata, dict):
                if signal.metadata.get("is_halted") is True:
                    is_halted = True
                elif signal.metadata.get("rejection_stage") in ("Halt", "Freeze", "CircuitBreaker"):
                    is_halted = True

            # 2. Structured DecisionEngine state directly if accessible
            if not is_halted and hasattr(self, "decision_engine") and hasattr(self.decision_engine, "spike_freeze_until"):
                if time.time() < getattr(self.decision_engine, "spike_freeze_until", 0):
                    is_halted = True

            # 3. Defensive string matching fallback
            if not is_halted and reason:
                r_upper = str(reason).upper()
                if "HALT" in r_upper or "FREEZE" in r_upper or "CHAOS" in r_upper:
                    is_halted = True

            decision_status = "HALTED" if is_halted else "REJECTED"
        ...
        summary = {
            ...
            "regime_confidence": signal.metadata.get("regime_conf", 0.0) if hasattr(signal, "metadata") and isinstance(signal.metadata, dict) else 0.0,
    ...
    def _record_v2_snapshot(self, signal, snapshot, outputs=None, filter_result=None, final_decision="REJECTED", rejection_reason="", timeline=None):
        ...
        try:
            ...
            # ── POST-COMMIT: Structure Reset Shadow Observer ──
            # Records prospective telemetry for every SR-rejected signal.
            # Architecturally incapable of order routing (no OMS import).
            if self.sr_shadow_observer and "SAME_STRUCTURAL_TREND" in (rejection_reason or ""):
                try:
                    structure_state = {}
                    if hasattr(self, 'trend_tracker'):
                        structure_state = self.trend_tracker.get_state()
                    self.sr_shadow_observer.observe(
                        snapshot_id=signal.id,
                        timestamp=metadata.timestamp,
                        signal_type=signal.signal_type.value if hasattr(signal.signal_type, 'value') else str(signal.signal_type),
                        direction=signal.direction.value if hasattr(signal.direction, 'value') else str(getattr(signal, 'direction', '')),
                        spot=getattr(snapshot, 'price', 0.0),
                        ...
                        stop_loss=getattr(signal, 'stop_loss', 0.0),
                        target_1=getattr(signal, 'target_1', 0.0),
                        gate_results={k: {"passed": v.passed} for k, v in snap_v2.gate_results.items()},
                        structure_details=structure_state,
                    )
                except Exception as sre:
                    logger.error(f"[SHADOW-SR] Observer failed (V2 unaffected): {sre}")
        except Exception as e:
            logger.error(f"Failed to record V2 snapshot: {e}")

    def evaluate(self, df: pd.DataFrame, snapshot: MarketSnapshot, cycle_count: int, latencies: Optional[dict] = None) -> DecisionPipelineResult:
        """
        ...
        # Diagnostic Invariant: Run strictly during active trade evaluation
        self._verify_regime_invariant(snapshot, signal)
        ...
        self._log_canonical_truth(signal, cycle_count, True, "", latencies)
