...
class DecisionEngine:
    ...
    def _process_impl(self, df, snapshot: "MarketSnapshot", historical_override = None) -> "Signal":
        ...
        meta = {
            ...
            "regime_conf": (
                float(outputs_dict.get("regime").confidence) if outputs_dict.get("regime") and hasattr(outputs_dict.get("regime"), "confidence")
                else float(snapshot.regime_state.get("confidence", 0.0) if hasattr(snapshot, "regime_state") and isinstance(snapshot.regime_state, dict) else 0.0)
            ),
    ...
    def _no_trade_signal(self, snapshot: MarketSnapshot, reasons: List[str], outputs: Dict[str, AgentOutput]) -> Signal:
        ...
        # Regime confidence extraction: outputs.regime -> snapshot.regime_state -> 0.0
        reg_conf = 0.0
        if outputs and "regime" in outputs and outputs["regime"] is not None:
            reg_conf = float(getattr(outputs["regime"], "confidence", 0.0))
        elif hasattr(snapshot, "regime_state") and isinstance(snapshot.regime_state, dict):
            reg_conf = float(snapshot.regime_state.get("confidence", 0.0))

        # Check structured halt status
        is_halted = (
            (hasattr(self, "spike_freeze_until") and time.time() < self.spike_freeze_until)
            or any("Halt:" in r or "Freeze" in r or "Chaos" in r for r in final_reasons)
        )
        ...
        signal = Signal(
            ...
            metadata={
                ...
                "regime_conf": reg_conf,
                "rejection_reason": final_reasons[0] if final_reasons else "NO_TRADE",
                "is_halted": is_halted,
            }
        )
    ...
    @staticmethod
    def _parse_market_regime(val: Any) -> Optional[MarketRegime]:
        """
        Safely convert a string or MarketRegime enum into a validated MarketRegime.
        Returns None if invalid, malformed, or empty.
        """
        if isinstance(val, MarketRegime):
            return val
        if isinstance(val, str) and val.strip():
            val_clean = val.strip().upper()
            try:
                return MarketRegime[val_clean]
            except KeyError:
                pass
            for member in MarketRegime:
                if member.value == val_clean:
                    return member
            # Compatible fallback aliases
            if "DOWN" in val_clean and "TREND" in val_clean:
                return MarketRegime.STRONG_TREND_DOWN if "STRONG" in val_clean else MarketRegime.TREND_DOWN
            if "UP" in val_clean and "TREND" in val_clean:
                return MarketRegime.STRONG_TREND_UP if "STRONG" in val_clean else MarketRegime.TREND_UP
            if "RANGE" in val_clean or "RANGING" in val_clean:
                return MarketRegime.RANGING
            if "VOLATILE" in val_clean:
                return MarketRegime.VOLATILE_CHOPPY
            if "BREAKOUT" in val_clean:
                return MarketRegime.BREAKOUT
            if "SQUEEZE" in val_clean:
    ...
    def _classify_market(self, snapshot: MarketSnapshot, outputs: Dict[str, AgentOutput]) -> MarketRegime:
        ...
        Converts raw indicators or regime manager states into human-readable canonical market archetypes.
        Precedence:
        1. RegimeAgent output (if available in outputs)
        2. RegimeStateManager snapshot.regime_state (defensively validated)
        3. Indicator heuristics (legacy fallback: Squeeze -> Choppy -> Breakout -> Ranging)
        """
        # 1. RegimeAgent output
        regime_agent = outputs.get("regime") if outputs else None
        if regime_agent and hasattr(regime_agent, "details") and isinstance(regime_agent.details, dict):
            reg_val = regime_agent.details.get("regime", "")
            parsed = self._parse_market_regime(reg_val)
            if parsed and parsed != MarketRegime.UNKNOWN:
                return parsed

        # 2. RegimeStateManager snapshot state (defensive validation)
        regime_state = getattr(snapshot, "regime_state", None)
        if isinstance(regime_state, dict):
            reg_val = regime_state.get("regime")
            parsed = self._parse_market_regime(reg_val)
            if parsed and parsed != MarketRegime.UNKNOWN:
                return parsed

        # 3. Indicator heuristics fallback
        if hasattr(snapshot, 'bb_width') and snapshot.bb_width > 0 and snapshot.bb_width < self.settings.thresholds.squeeze_threshold:
            return MarketRegime.SQUEEZE
            
        bulls = sum(1 for o in outputs.values() if getattr(o, "direction", None) == Direction.BULLISH) if outputs else 0
        bears = sum(1 for o in outputs.values() if getattr(o, "direction", None) == Direction.BEARISH) if outputs else 0
        if bulls > 2 and bears > 2:
            return MarketRegime.VOLATILE_CHOPPY
                
        if hasattr(snapshot, 'bb_upper') and hasattr(snapshot, 'bb_lower'):
            if snapshot.price > snapshot.bb_upper or snapshot.price < snapshot.bb_lower:
                return MarketRegime.BREAKOUT
