...
Decay schedule (default params):
  Time after open   Remaining penalty
  ─────────────────────────────────
  9:15 (open)       100%
  9:45 (30m)        ~45%
  10:15 (60m)       ~20%
  11:15 (120m)      ~4%  ← effectively zero
  12:00+            negligible
...
# ── Decay constant: chosen so penalty reaches ~5% after 2 hours ──
# k = -ln(0.05) / 120  ≈ 0.025
...
# moderate: ≤0.5 → 0.15 cap
# significant: ≤0.8 → 0.22 cap
# major: >0.8    → 0.30 cap  (still NOT a kill-switch)
_GAP_SEVERITY_THRESHOLDS = [
    (0.25, 0.08, "MINOR"),
    (0.50, 0.15, "MODERATE"),
    (0.80, 0.22, "SIGNIFICANT"),
    (float("inf"), 0.30, "MAJOR"),
]
...
class GapPenaltyManager:
    ...
    def get_unified_penalty_multiplier(
        ...
        """
        Returns a 0.70–1.00 MULTIPLIER (not additive) for score scaling.
        This is the format consumed by compute_weighted_score().

        e.g. penalty=0.15 → multiplier=0.85
        """
        return round(1.0 - self.get_penalty(minutes_since_open), 4)
