...
class Settings:
    ...
    def __init__(self):
        ...
        # ── Risk Override (Strict Fail-Closed Validation) ──
        raw_mdl = float(os.getenv("MAX_DAILY_LOSS", 3000.0))
        if raw_mdl <= 0:
            raise ValueError(
                f"Configuration Error: MAX_DAILY_LOSS must be a positive number representing "
                f"the maximum allowable loss limit in Rupees (got {raw_mdl}). E.g., MAX_DAILY_LOSS=3000.0"
            )
        max_daily_loss = raw_mdl

        raw_mtl = float(os.getenv("MAX_LOSS_PER_TRADE", 500.0))
        if raw_mtl <= 0:
            raise ValueError(
                f"Configuration Error: MAX_LOSS_PER_TRADE must be a positive number (got {raw_mtl})."
            )
        max_trade_loss = raw_mtl
