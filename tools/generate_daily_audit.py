...
def _load_execution_metrics(target_date: str) -> dict:
    """
    Dedicated loader for session-level engine activity from execution_metrics.jsonl.
    Canonical source of truth for:
    - Market evaluation cycles (deduplicated by engine_cycle_id)
    - Halt / freeze activity and reasons
    - Canonical execution truth and decision_status
    - Execution authorizations
    """
    metrics_path = WORKSPACE / "data" / "execution_metrics.jsonl"
    ...
    with open(metrics_path, "r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            ...
            total_records += 1
            cid = rec.get("engine_cycle_id")
            if cid is None:
                cid = f"rec_{total_records}"
            if cid not in cycles:
                cycles[cid] = []
            cycles[cid].append(rec)

    evaluation_cycles = len(cycles)
    if evaluation_cycles == 0:
        return {
            "has_data": False,
            "total_records": 0,
            "evaluation_cycles": 0,
    ...
    for cid, recs in cycles.items():
        is_auth = any(r.get("execution_authorized") is True for r in recs)
        is_halt = any(
            r.get("decision_status") == "HALTED" or
            r.get("is_halted") is True
            for r in recs
        )
        if not is_auth and not is_halt:
            if any(
                r.get("opportunity_valid") is False and (
                    "HALT" in str(r.get("kill_reason", "")).upper() or
                    "FREEZE" in str(r.get("kill_reason", "")).upper() or
                    "HALT" in str(r.get("rejection_reason", "")).upper() or
                    "FREEZE" in str(r.get("rejection_reason", "")).upper()
                )
                for r in recs
            ):
                is_halt = True
        ...
        reasons = [
            r.get("kill_reason") or r.get("rejection_reason")
            for r in recs
            if (r.get("kill_reason") or r.get("rejection_reason"))
        ]
        if reasons:
            primary_reason = reasons[-1]
            cycle_reasons[primary_reason] += 1
            if "FREEZE" in primary_reason.upper() or "HALT" in primary_reason.upper():
                freeze_reasons[primary_reason] += 1

        regime_vals = [r.get("regime") for r in recs if r.get("regime")]
        if regime_vals:
            regimes[regime_vals[-1]] += 1

    is_safety_freeze = (
        halted_cycles > 0 and
        authorized_cycles == 0 and
        halted_cycles == evaluation_cycles
    )
    ...
    return {
        ...
        "authorized_cycles": authorized_cycles,
        "rejected_cycles": rejected_cycles,
        "freeze_reasons": dict(freeze_reasons.most_common()),
        "cycle_reasons": dict(cycle_reasons.most_common()),
        "regimes": dict(regimes.most_common()),
        "is_safety_freeze": is_safety_freeze,
    }
    ...
    if log_path.exists():
        with open(log_path, "r", encoding="utf-8", errors="ignore") as f:
            for line in f:
                ...
                # Rate limits: ensure we don't match volume numbers like 80517.0 on INFO lines
                if ("RATE_LIMIT" in upper or "RATE LIMIT" in upper or re.search(r'\b805\b', line)) and ("ERROR" in upper or "WARN" in upper or "API" in upper):
                    rate_limits += 1
                    infra_degraded = True
...
def _get_campaign_progress(target_date: str, current_overall_status: str) -> dict:
    if not DB_PATH.exists():
        return {
            "valid": 0, "completed": 0, "economic_valid": 0, "capture_valid": 0,
            "observed": 0, "excluded": 0, "invalid": 0, "required": 20, "remaining": 20
        }

    # Discover dates from decision_snapshots_v2 and execution_metrics.jsonl
    dates = set()
    conn = sqlite3.connect(str(DB_PATH))
    try:
        df = pd.read_sql_query("SELECT DISTINCT substr(timestamp, 1, 10) as d FROM decision_snapshots_v2", conn)
        dates.update(df['d'].tolist())
    except Exception:
        pass
    finally:
        conn.close()
    ...
    for d in sorted(dates):
        ...
        if "INVALID" in status or "FAILURE" in status:
            invalid_count += 1
    ...
    return {
        "valid": economic_valid_count,
        "completed": economic_valid_count,
        "economic_valid": economic_valid_count,
        "capture_valid": capture_valid_count,
        "observed": observed,
        "excluded": excluded_count,
        "invalid": invalid_count,
        "required": 20,
        "remaining": max(0, 20 - economic_valid_count)
    }
...
def generate_daily_audit(target_date: str = None):
    ...
    exec_metrics = _load_execution_metrics(target_date)
    ...
    # Two-source truth: session has valid data if either candidate snapshots were recorded OR engine evaluation cycles were captured
    data_ok = len(df_snaps) > 0 or exec_metrics["has_data"]
    ...
    # Structured check: did execution freeze safely hold throughout evaluation cycles?
    is_safety_halted = exec_metrics["is_safety_freeze"] and len(df_snaps) == 0
    ...
    elif infra_degraded and (is_halted or is_safety_halted):
        overall_status = "🟡 DEGRADED + HALTED + EXCLUDED"
    elif infra_degraded:
        overall_status = "🟡 DEGRADED (EXCLUDED)"
    elif is_safety_halted:
        overall_status = "🟡 HALTED — SAFETY FREEZE ACTIVE"
    elif is_halted:
    ...
    if has_fatal:
        ...
        decision_icon = "🟡"
        decision_label = "OBSERVED — CAPTURE VALID (SAFETY FREEZE)"
        decision_reason = (
            f"Market evaluation was active with {exec_metrics['evaluation_cycles']:,} unique engine cycles. "
            "The safety freeze suppressed directional candidate generation. "
            "Zero OMS/execution calls occurred. Economic replay opportunities were therefore zero, "
            "but this was not a data-feed or capture failure."
        )
    elif infra_degraded and (is_halted or is_safety_halted):
        decision_icon = "🟡"
    ...
    market_eval_cycles = exec_metrics["evaluation_cycles"] if exec_metrics["evaluation_cycles"] > 0 else signals["total"]
    ...
    lines.append(f"| :--- | ---: |")
    lines.append(f"| Market Evaluations (Engine Cycles) | {market_eval_cycles:,} |")
    lines.append(f"| Directional Candidates (V2 Snapshots) | {signals['total']} |")
    ...
    lines.append(f"| Execution Authorized Cycles | {exec_metrics['authorized_cycles']} |")
    lines.append(f"| Counterfactual Replay Expectancy | {replay_expectancy:+.2f}R |")
    lines.append(f"| System Posture | {'🟡 SAFETY FREEZE ACTIVE' if is_safety_halted else ('🔴 ' + sys_state.get('state') if is_halted else '🟢 ACTIVE')} |")
    ...
    lines.append(f"| Campaign Progress | {campaign['economic_valid']} / {campaign['required']} valid ({campaign['observed']} observed, {campaign['capture_valid']} capture-valid, {campaign['excluded']} excluded) |")
    lines.append("")
    ...
    lines.append(f"| :--- | ---: | :--- |")
    lines.append(f"| 1. Market Evaluations (Engine Cycles) | {market_eval_cycles:,} | Telemetry source of truth: execution_metrics.jsonl |")
    if is_safety_halted:
        lines.append(f"| ├── Safety / Freeze Suppressed Cycles | {exec_metrics['halted_cycles']:,} | Suppressed by Phase 1/Phase 2 safety halts |")
        lines.append(f"| └── Directional Candidates Generated | 0 | Candidate source of truth: decision_snapshots_v2 |")
    else:
    ...
    if exec_metrics.get("cycle_reasons"):
        lines.append("**Engine Evaluation / Safety Gate Breakdown**:")
        lines.append("")
        lines.append(f"| Safety Gate / Reason | Count (Cycles) | % of Cycles |")
        lines.append(f"| :--- | ---: | ---: |")
        tot_eval = exec_metrics["evaluation_cycles"]
        for r_name, r_cnt in exec_metrics["cycle_reasons"].items():
            pct = (r_cnt / tot_eval * 100) if tot_eval > 0 else 0
            lines.append(f"| {r_name} | {r_cnt:,} | {pct:.1f}% |")
        lines.append(f"| **Total Engine Cycles** | **{tot_eval:,}** | **100.0%** |")
        lines.append("")

    if exec_metrics.get("regimes"):
        lines.append("**Observed Market Regimes (Engine Telemetry)**:")
        lines.append("")
        lines.append(f"| Regime | Cycles | % of Session |")
        lines.append(f"| :--- | ---: | ---: |")
        tot_eval = exec_metrics["evaluation_cycles"]
        for reg_name, reg_cnt in exec_metrics["regimes"].items():
            pct = (reg_cnt / tot_eval * 100) if tot_eval > 0 else 0
            lines.append(f"| {reg_name} | {reg_cnt:,} | {pct:.1f}% |")
        lines.append(f"| **Total Cycles** | **{tot_eval:,}** | **100.0%** |")
        lines.append("")
    ...
    lines.append(f"| Historical Replay Dataset | 🟡 ACCUMULATING ({campaign['economic_valid']}/{campaign['required']} economic sessions, {campaign['capture_valid']} capture-valid) |")
    ...
    if errors["fatal"] > 0:
        ...
        lines.append(f"**Operational Health:** 🟢 Nominal ({exec_metrics['evaluation_cycles']:,} cycles evaluated without runtime errors)")
        lines.append(f"**Safety Status:** 🟡 Safety freeze was active (Phase 2 Intraday Spike Freeze)")
        lines.append("**Validation Limitation:** Zero directional candidates produced due to active freeze; session serves as capture validation, not economic validation")
    ...
    lines.append(f"| Metric | Value |")
    lines.append(f"| :--- | ---: |")
    lines.append(f"| Replay Sessions (Economic Valid) | {campaign['economic_valid']} / {campaign['required']} |")
    lines.append(f"| Capture-Valid Observed Sessions | {campaign['capture_valid']} |")
    ...
    lines.append("")
    if is_safety_halted:
        lines.append(f"**Session {target_date} represents a valid observed safety session.**")
        lines.append("")
        freeze_cycle_count = exec_metrics.get('cycle_reasons', {}).get('Phase 2 Halt: Intraday Spike Freeze active', 0)
        lines.append(f"The trading engine operated continuously across **{exec_metrics['evaluation_cycles']:,} unique market evaluation cycles**. All cycles were safely intercepted by safety gates ({freeze_cycle_count:,} cycles in Intraday Spike Freeze). No directional candidates entered the candidate evaluation layer, and zero OMS orders or executions were attempted.")
        lines.append("")
        lines.append("This session is confirmed as **Capture-Valid** (telemetry recording, regime tracking, and safety gate suppression functioned flawlessly). However, because zero candidate trade setups were produced, it provides zero counterfactual replay data and is **not counted toward the 20 economic-validation sessions** required for strategy deployment readiness.")
        lines.append("")
    else:
    ...
    lines.append(f"**Campaign Progress**: {campaign['economic_valid']} / {campaign['required']} economic-valid ({campaign['capture_valid']} capture-valid)")
    lines.append("")
    lines.append("---")
